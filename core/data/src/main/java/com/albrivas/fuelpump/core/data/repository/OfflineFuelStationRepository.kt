...
class OfflineFuelStationRepository @Inject constructor(
    ...
    override val listFuelStation: Flow<List<String>> =
        fuelStationDao.getFuelStations().map { items -> items.map { it.locality } }


    override suspend fun addAllStations(listStations: List<FuelStationEntity>) {
