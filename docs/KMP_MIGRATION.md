...
| 9F-9I | (pendiente) | `:core:ui`, `:core:analytics`, `:core:notifications` | **Android + Android Auto + iOS funcional (paridad completa)** |
...
**Estado actual (post Phase 9F)**: las Phases 0-8D están completadas. Las sub-fases 9A, 9B, 9C (MVP), 9D, 9E y 9F están completadas — la app iOS ya muestra el mapa con estaciones tap-eables, polyline de ruta, punto de ubicación, búsqueda de lugares funcional, acciones del detalle (abrir Maps, compartir, pedir permiso de notificaciones) y review nativa de App Store al marcar favorito. Stubs cerrados: #1 NetworkMonitor, #2 Geocoder, #3 LocationTracker, #4 PlacesRepository, #5 PlatformMapView, #6 LocationPermission, #7 MapsNavigation, #8 ShareAction, #9 NotificationPermission, #10 InAppReviewManager (10 de 12). Quedan 2 stubs activos: #11 Analytics Mixpanel, #12 OneSignal push. El siguiente paso inmediato es **Phase 9C.2** (map polish iOS — clustering + markers visuales) o **Phase 9G** (Analytics Mixpanel iOS). Tras Phase 9, abordar **Phase 8E** (mover `initKoin` Android a `composeApp/androidMain`, simetría con iOS) y opcionalmente **Phase 8** (`jvm()` target global para tests sin emulador).