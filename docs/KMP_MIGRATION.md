...
| 9H-9I | (pendiente) | `:core:notifications` | **Android + Android Auto + iOS funcional (paridad completa)** |
...
**Estado actual (post Phase 9G)**: las Phases 0-8D están completadas. Las sub-fases 9A, 9B, 9C (MVP), 9D, 9E, 9F y 9G están completadas — la app iOS muestra el mapa con estaciones tap-eables, polyline de ruta, punto de ubicación, búsqueda de lugares funcional, acciones del detalle (abrir Maps, compartir, pedir permiso de notificaciones), review nativa de App Store y métricas reales de Mixpanel. Stubs cerrados: #1 NetworkMonitor, #2 Geocoder, #3 LocationTracker, #4 PlacesRepository, #5 PlatformMapView, #6 LocationPermission, #7 MapsNavigation, #8 ShareAction, #9 NotificationPermission, #10 InAppReviewManager, #11 Analytics Mixpanel (11 de 12). Queda 1 stub activo: #12 OneSignal push. La Phase 9G usó un patrón nuevo: `@ObjCName` en la interfaz Kotlin → protocolo ObjC en ComposeApp → Swift lo implementa con el SDK oficial, sin ningún cinterop. El `KoinInit.kt` iOS ahora acepta `AnalyticsHelper` como parámetro externo. El siguiente paso es **Phase 9C.2** (map polish iOS) o **Phase 9H** (OneSignal push, OPCIONAL). Tras Phase 9, abordar **Phase 8E** (mover `initKoin` Android a `composeApp/androidMain`, simetría con iOS) y opcionalmente **Phase 8** (`jvm()` target global para tests sin emulador).