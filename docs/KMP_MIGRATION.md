...
| 9 | feature/kmp-phase9-ios-parity (sub-PRs 9A-9I) | `:core:data`, `:core:analytics`, `:core:notifications`, `:core:ui`, `:feature:station-map`, `:feature:detail-station` | **Android + Android Auto + iOS funcional (paridad)** |
...
|---|--------|------------|-------------------------|---------|
| 1 | `:core:data` NetworkMonitor | `flowOf(true)` siempre | Detección de conectividad real | UX offline degradada |
| 2 | `:core:data` Geocoder | `flowOf(null)` | Reverse geocoding (coords → dirección) | Sin direcciones legibles |
| 3 | `:core:data` LocationTracker | `null`/false en flows | Ubicación del usuario + permisos | **BLOQUEANTE** — sin ubicación, sin estaciones cercanas |
| 4 | `:core:data` PlacesRepository | `emptyList()` | Búsqueda de lugares | Búsqueda no funciona |
| 5 | `:feature:station-map` PlatformMapView | `Box` con color de fondo | Mapa interactivo con marcadores | **BLOQUEANTE** — mapa visible pero no renderiza |
| 6 | `:feature:station-map` LocationPermission | `isGranted=true` siempre | Pedir permiso de ubicación | Permiso no pedido al usuario |
...
9B LocationTracker + permisos       [CoreLocation + Info.plist]
   ↓
9C Mapa interactivo                 [MapKit vs Google Maps SDK iOS — DECIDIR]
   ↓
...
### Phase 9B — Location + permisos (CLLocationManager)

**Stubs a cerrar**: #3 LocationTracker, #6 LocationPermission.

**Por qué después de 9A**: requiere manejo de permisos (`Info.plist`, `CLLocationManagerDelegate`). Bloqueante para Phase 9C (el mapa necesita ubicación inicial).

**Investigación previa requerida**:
- ¿`CLLocationManagerDelegate` como `NSObject` Kotlin/Native o como Swift bridge expuesto a Kotlin? Trade-off: simplicidad de mantenimiento vs idiomática iOS.
- ¿Cómo manejar el ciclo de vida del delegate desde un `callbackFlow` (evitar retain cycles con `awaitClose`)?
- Permission flow: ¿pedir al primer uso (lazy) o al onboarding (eager)? Android lo hace eager en onboarding — mantener simetría.
- Copy de `NSLocationWhenInUseUsageDescription` (es/en, alineado con Android).

**Archivos previstos**:
- `core/data/src/iosMain/.../repository/location/LocationTrackerIos.kt`
- `feature/station-map/src/iosMain/.../platform/LocationPermission.kt`
- `iosApp/iosApp/Info.plist` (claves de permisos)

**Verificación**: simulador pide permiso en onboarding; denegado muestra UX correcta; aceptado centra mapa.

---

### Phase 9C — Mapa interactivo

**Stub a cerrar**: #5 PlatformMapView.

**Investigación previa requerida (DECISIÓN GRANDE)**:
- **Opción A — MapKit nativo (`UIKitView { MKMapView() }`)**: sin coste, sin SDK externo, integración mínima. Estética distinta a Android (Google Maps), markers personalizados más limitados, clustering manual.
- **Opción B — Google Maps SDK iOS (pod `GoogleMaps`)**: paridad visual exacta con Android, reutiliza la API key Google ya configurada, marker rendering uniforme, clustering oficial. Añade pod + tamaño binario significativo + dependencia Google.
- **Criterio**: si la UX del mapa es crítica para la marca y queremos que iOS se vea idéntico a Android → Google Maps SDK iOS. Si se prioriza cero deps externas → MapKit.

Cierre de la decisión: documentar en `docs/KMP_PHASE9C.md` con tabla comparativa al iniciar la sub-fase.

**Archivos previstos**:
- `feature/station-map/src/iosMain/.../platform/PlatformMapView.kt`
- Posible `build.gradle.kts` de `:feature:station-map` o `:composeApp` (pod `GoogleMaps` si opción B)
- `iosApp/iosApp/Info.plist` (`GMSServices.provideAPIKey` si opción B)

**Verificación**: mapa muestra estaciones, soporta pan/zoom, clicks abren detalle.
...
**Estado actual (post Phase 8D)**: las Phases 0-8D están completadas. La app iOS arranca, navega y muestra el shell pero está funcionalmente vacía (12 stubs no-op). El siguiente bloque es **Phase 9 — iOS Feature Parity** (ver sección dedicada). Cada sub-fase 9A-9I es una PR independiente y requiere investigación técnica previa para decidir el enfoque (ej. MapKit vs Google Maps SDK iOS). Tras Phase 9, abordar **Phase 8E** (mover `initKoin` Android a `composeApp/androidMain`, simetría con iOS) y opcionalmente **Phase 8** (`jvm()` target global para tests sin emulador).