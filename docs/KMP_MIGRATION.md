...
**Estado actual (post Phase 9H)**: las Phases 0-8D están completadas. Las sub-fases 9A, 9B, 9C (MVP), 9D, 9E, 9F, 9G y 9H están completadas — **los 12 stubs de iOS están cerrados**. La app iOS tiene: mapa con estaciones tap-eables, polyline de ruta, punto de ubicación, búsqueda de lugares (Google Places), acciones del detalle (abrir Maps, compartir, pedir permiso de notificaciones), review nativa de App Store, métricas reales de Mixpanel y push notifications via OneSignal. La Phase 9H reutilizó el patrón Swift bridge de 9G: `@ObjCName` en las interfaces Kotlin → protocolos ObjC en ComposeApp → clases Swift las implementan con el SDK OneSignal. `NotificationService.init()` renombrado a `start()` para evitar colisión con keyword Swift. `initKoin()` ahora devuelve `DeepLinkStateHolder` directamente desde el retorno de `startKoin {...}.koin.get()`. El siguiente paso es **Phase 9I** (validación E2E: ejercitar todos los flujos en iPhone físico, documentar bugs, escribir `docs/KMP_PHASE9.md`) o **Phase 9C.2** (map polish: custom markers con precio/logo + clustering) o **Phase 8E** (mover `initKoin` Android a `composeApp/androidMain`, simetría con iOS).